...
## Load users` fasta files
  loadProbeSeq <- reactive({ 
    ...
    #seqfa <- read.table(input$probeseq$datapath,
    #                   sep = "\n")
    #
...
## Load Genome
  ...
  output$showGenomePath <- renderText(
    loadUsrGenome()
  )
...
## Load GTF         
  loadUsrGTF <- reactive({ 
    req(input$gtf)
    gtfFile <- data.table::fread(input$gtf$datapath,
                        sep = "\t")
    return(gtfFile)
})
...
getBowtieIndex <- reactive({
  req(loadUsrGenome())
  indexDir <- tempdir()
  tmp <- Rbowtie::bowtie_build(references=loadUsrGenome(), outdir=indexDir, prefix="index", force=TRUE)
  return(indexDir)
  })  
...
getBowtieAlign <- reactive({
  ...
  samFile <- tempfile()
  Rbowtie::bowtie(sequences = readsFiles, 
         index=file.path(getBowtieIndex(), "index"), 
...
getAnnotation <- reactive({
  ...
  lo=cbind(as.data.frame(convertBamToGR()[queryHits(o)]),
           as.data.frame(preprocessGTF()[subjectHits(o)]))
  #head(lo)
  return(lo)
})
